...
public class Board extends JPanel
{
  ...
  */
  ...
  public void paint(Graphics g)
  {
    ...
    /* Detect collisions */
    oops = thereIsACollision();

    /* Kill the pacman */
    killThePacMan(g, oops);

    /* Delete the players and ghosts */
    deleteThePlayersAndGhosts(g);
    ...
    if ( pellets[player.pelletX][player.pelletY] && newGame !=2 && newGame !=3)
    {
      ...
      if (player.pelletsEaten == 173)
      {
        ...
        generateScreenAccordingToModeAndScore();
        return;
      }
    }

    /* If we moved to a location without pellets, stop the sounds */
    else if (locationWithoutPellets())
    {
      /* Stop any pacman eating sounds */
      sounds.nomNomStop();
    }
  ...
  private void optimiseDrawing(Graphics g) {
  ...
  private void deleteThePlayersAndGhosts(Graphics g) {
    g.setColor(Color.BLACK);
  ...
  private void drawThePacMan(Graphics g) {
  ...
  private void drawTheGhosts(Graphics g) {
  ...
  private boolean locationWithoutPellets() {
    return (player.pelletX != lastPelletEatenX || player.pelletY != lastPelletEatenY) || player.stopped;
  }

  private void generateScreenAccordingToModeAndScore() {
    if (!demo)
    {
  ...
  private void drawMouthClosed(Graphics g) {
    /* Draw mouth closed */
    g.drawImage(pacmanImage,player.x,player.y,Color.BLACK,null);
  }

  private void drawMouthOpen(Graphics g) {
  ...
  private void drawScreen(Graphics g, Image gameOverImage) {
    g.setColor(Color.BLACK);
    g.fillRect(0,0,600,600);
  ...
  private void clearHightScores(Graphics g) {
    g.setColor(Color.BLACK);
  ...
  private void generatethirdFrame() {
    newGame++;
  ...
  private void generateFirstFrame(Graphics g) {
    reset();
  ...
  private boolean thereIsACollision() {
    return ((player.x == ghost1.x && Math.abs(player.y - ghost1.y) < 10) ||
            (player.x == ghost2.x && Math.abs(player.y - ghost2.y) < 10) ||
            (player.x == ghost3.x && Math.abs(player.y - ghost3.y) < 10) ||
            (player.x == ghost4.x && Math.abs(player.y - ghost4.y) < 10) ||
